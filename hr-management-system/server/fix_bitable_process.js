...
// Insert processBadReviewData if it doesn't exist
if (!content.includes('processBadReviewData')) {
  const badReviewFunc = `
...
`;
  content = content.replace(/(async function processTableVisitData.*?\n})/s, '$1\n' + badReviewFunc);
  
  // Add to switch case
  content = content.replace(/case 'table_visit':\s+return await processTableVisitData\(records\);/, 
