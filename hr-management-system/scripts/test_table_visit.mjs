...
// 桌访表配置
const TABLEVISIT_APP_ID = 'cli_a9fc0d13c838dcd6';
const TABLEVISIT_APP_SECRET = 'pRVuBmiWc0hzqP1YzZDqzGUPFlaProDN';
const TABLEVISIT_APP_TOKEN = 'PTWrbUdcbarCshst0QncMoY7nKe';
const TABLEVISIT_TABLE_ID = 'tblpx5Efqc6eHo3L';

console.log('🔧 桌访表配置信息:');
console.log('App ID:', TABLEVISIT_APP_ID);
console.log('App Secret:', TABLEVISIT_APP_SECRET.substring(0, 10) + '...');
console.log('App Token:', TABLEVISIT_APP_TOKEN);
console.log('Table ID:', TABLEVISIT_TABLE_ID);

async function getTableVisitTenantToken() {
  try {
    const resp = await axios.post('https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal', {
      app_id: TABLEVISIT_APP_ID,
      app_secret: TABLEVISIT_APP_SECRET
    }, { timeout: 10000 });
...
async function testTableVisitConnection() {
  console.log('🚀 开始测试桌访表连接...');

  // 1. 获取 Token
  const token = await getTableVisitTenantToken();
  if (!token) {
  ...
  try {
    const resp = await axios.get(
      `https://open.feishu.cn/open-apis/bitable/v1/apps/${TABLEVISIT_APP_TOKEN}/tables/${TABLEVISIT_TABLE_ID}/records`,
      {
...
// 运行测试
testTableVisitConnection().catch(console.error);
