...
async function fetchTableFields(tableConfig, accessToken) {
  try {
    ...
    if (response.data.code === 0) {
      ...
      // 显示主要字段
      const mainFields = fields.slice(0, 10);
      console.log(`   主要字段:`);
...
function showSampleData(tableName, records) {
  ...
  records.slice(0, 2).forEach((record, index) => {
    ...
    const sampleFields = Object.keys(fields).slice(0, 5);
...
async function testTable(tableKey, tableConfig, accessToken) {
  ...
  return {
    ...
    fieldNames: fieldResult.success ? fieldResult.fields?.map(f => f.field_name) : [],
    sampleData: dataResult.success ? dataResult.data?.items?.slice(0, 2) : []
  };
}

// 主测试函数
async function runAllTablesTest() {
  console.log(`🚀 重新测试HRMS与飞书表格连通性（权限已修复）`);
  ...
  if (successfulTables.length > 0) {
    console.log(`\n✅ 成功连通的表格详情:`);
    successfulTables.forEach(result => {
      console.log(`\n📊 ${result.table}:`);
      console.log(`   记录数: ${result.records}`);
      console.log(`   字段数: ${result.fields}`);
      if (result.fieldNames.length > 0) {
        console.log(`   主要字段: ${result.fieldNames.slice(0, 5).join(', ')}`);
        if (result.fieldNames.length > 5) {
          console.log(`               ... 还有${result.fieldNames.length - 5}个字段`);
        }
      }
    });
  }
  ...
  if (successCount === totalCount) {
    ...
    console.log(`✅ 所有表格已成功对接完成`);
  } else if (successCount > 0) {
    console.log(`⚠️  部分表格测试成功! ${successCount}/${totalCount} 个表格可用`);
    console.log(`💡 成功的表格可以正常使用，失败的表格需要进一步检查`);
  } else {
    console.log(`❌ 所有表格测试失败! 连通性仍存在问题`);
    console.log(`💡 建议检查权限配置或联系技术支持`);
  }
  
  console.log(`\n🔧 对接状态:`);
  results.forEach(result => {
    const status = result.success ? '✅ 已对接' : '❌ 未对接';
