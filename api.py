...
class PortfolioReturnsResponse(BaseModel):
    ...
    agent_a: "PortfolioSourceReturnsResponse" = Field(default_factory=lambda: PortfolioSourceReturnsResponse())
    agent_b: "PortfolioSourceReturnsResponse" = Field(default_factory=lambda: PortfolioSourceReturnsResponse())
...
def _create_trade_at_price(position_id: str, side: str, quantity: float, price: float, source: str, session=None) -> Optional[PortfolioTradeResponse]:
    ...
    if owns_session:
        with session_scope() as s:
            trade = _create_trade_at_price(position_id, side, quantity, price, source, session=s)
        if trade is not None:
            _send_feishu_trade_notify(trade.position_id, trade.side, trade.price, trade.quantity, trade.symbol, trade.name, trade.market, trade.source)
        return trade

    p = session.get(PortfolioPosition, position_id)
    if not p:
        return None
...
def _agent_id_to_source(agent_id: str) -> str:
...
def _queue_auto_trade(session, position_id: str, side: str, trigger_price: float, quantity: float, source: str) -> Optional[PortfolioAutoTradeResponse]:
    ...
    qty = float(quantity or 0.0)
    if trigger <= 0 or qty <= 0:
        return None
    p = session.get(PortfolioPosition, position_id)
    if not p:
        return None
    existing = session.execute(select(PortfolioAutoTrade).where(
        PortfolioAutoTrade.position_id == position_id,
        PortfolioAutoTrade.side == side,
        PortfolioAutoTrade.status == "PENDING",
        PortfolioAutoTrade.source == source,
    )).scalars().first()
    now = int(time.time())
    ...
    return PortfolioAutoTradeResponse(
        ...
        position_id=position_id,
        side=side,
        trigger_price=at.trigger_price,
        quantity=at.quantity,
        status=at.status,
        source=at.source,
        created_at=at.created_at,
        executed_at=at.executed_at,
        executed_price=at.executed_price,
        symbol=p.symbol,
        name=p.name,
        market=p.market,
    )
...
def _compute_agent_month_buy_amount(trades: list[PortfolioTrade], agent_id: str = "a") -> float:
    now_bj = _dt.datetime.now(_ZoneInfo("Asia/Shanghai"))
    month_start_ts = int(now_bj.replace(day=1, hour=0, minute=0, second=0, microsecond=0).timestamp())
    total = 0.0
    for trade in trades:
        if _trade_source_bucket(getattr(trade, "source", "manual") or "manual") != agent_id:
            continue
        if (trade.side or "").strip().upper() != "BUY":
            continue
        if int(getattr(trade, "created_at", 0) or 0) < month_start_ts:
            continue
        total += float(getattr(trade, "amount", 0.0) or 0.0)
    return total
...
def _compute_agent_managed_financials(trades: list[PortfolioTrade], positions: list[PortfolioPosition], agent_id: str = "a", capital: float = 10000000.0) -> tuple[float, float, float, float]:
    pos_prices: dict[str, float] = {}
    ...
    agent = breakdown.get(agent_id, _empty_source_summary())
...
def _compute_agent_period_returns(
    trades: list[PortfolioTrade],
    positions: list[PortfolioPosition],
    pos_prices: dict[str, float],
    agent_id: str,
    ...
    for position_id, items in by_position.items():
        ...
        for lot in lots:
            ...
            unrealized += mv - cost
    return {
        "today": realized["today"] + unrealized,
        "week": realized["week"] + unrealized,
        "month": realized["month"] + unrealized,
        "unrealized": unrealized,
    }


def _compute_agent_position_unrealized_pnl(
    positions: list[PortfolioPosition],
    holdings: dict[str, dict[str, float]],
    pos_prices: Optional[dict[str, float]],
    agent_id: str,
) -> float:
    live_position_ids = {
        position_id
    ...
    total = 0.0
    for p in positions:
        if p.id not in live_position_ids:
            continue
        qty = float(p.quantity or 0.0)
        current_price = float((pos_prices or {}).get(p.id, 0.0) or 0.0)
        avg_cost = float(p.avg_cost or 0.0)
...
def _compute_period_returns(
    ...
    holdings: dict[str, dict[str, float]],
    trades: list[PortfolioTrade],
    pos_prices: Optional[dict[str, float]] = None,
    ...
    agent_periods = {
        "a": _compute_agent_period_returns(trades, positions, pos_prices or {}, "a", today_ts, week_ts, month_ts),
        "b": _compute_agent_period_returns(trades, positions, pos_prices or {}, "b", today_ts, week_ts, month_ts),
    }
    unrealized_pnl_by_bucket = {
        "manual": float(summary.manual.unrealized_pnl or 0.0),
    ...
    for aid in ("a", "b"):
        try:
            with session_scope() as s:
                cfg = s.get(PortfolioAgentConfig, aid)
                if cfg:
                    if aid == "a":
                        agent_a_capital = float(cfg.capital or 10000000.0)
                    else:
                        agent_b_capital = float(cfg.capital or 10000000.0)
        except Exception:
            pass
    if agent_a_capital <= 0:
        agent_a_capital = 10000000.0
    if agent_b_capital <= 0:
        agent_b_capital = 10000000.0

    today_pnl = bucket_periods["manual"]["today"] + bucket_periods["a"]["today"] + bucket_periods["b"]["today"]
    week_pnl = bucket_periods["manual"]["week"] + bucket_periods["a"]["week"] + bucket_periods["b"]["week"]
    month_pnl = bucket_periods["manual"]["month"] + bucket_periods["a"]["month"] + bucket_periods["b"]["month"]
    ...
    return PortfolioReturnsResponse(
        ...
        agent_a=PortfolioSourceReturnsResponse(
            today_pnl=float(bucket_periods["a"]["today"]),
            week_pnl=float(bucket_periods["a"]["week"]),
            month_pnl=float(bucket_periods["a"]["month"]),
            total_pnl=agent_a_total,
            realized_pnl=float(summary.agent_a.realized_pnl or 0.0),
            unrealized_pnl=float(unrealized_pnl_by_bucket["a"]),
            net_return_rate=(agent_a_total / agent_a_capital * 100.0) if agent_a_capital > 0 else 0.0,
        ),
        agent_b=PortfolioSourceReturnsResponse(
            today_pnl=float(bucket_periods["b"]["today"]),
            week_pnl=float(bucket_periods["b"]["week"]),
            month_pnl=float(bucket_periods["b"]["month"]),
            total_pnl=agent_b_total,
            realized_pnl=float(summary.agent_b.realized_pnl or 0.0),
            unrealized_pnl=float(unrealized_pnl_by_bucket["b"]),
            net_return_rate=(agent_b_total / agent_b_capital * 100.0) if agent_b_capital > 0 else 0.0,
        ),
    )
...
def _get_or_create_agent_config(session, agent_id: str = "a") -> PortfolioAgentConfig:
    ...
    if cfg:
        return cfg
    now = int(time.time())
    cfg = PortfolioAgentConfig(id=agent_id, agent_type="rules" if agent_id == "a" else "llm", created_at=now, updated_at=now)
    session.add(cfg)
    session.flush()
    return cfg
...
def _portfolio_agent_pick_candidate(min_buy_quantity: float) -> Optional[tuple[str, str, str, float, str]]:
    items = _portfolio_agent_pick_candidates(min_buy_quantity, limit=1)
...
def _normalize_llm_actions(decision: dict) -> list[dict]:
    ...
    if isinstance(actions, list):
        ...
        for item in actions[:5]:
            ...
            result.append({
                ...
                "reason": str(item.get("reason") or "")[:50],
            })
        if result:
            return result
    return [{
        ...
        "reason": str(decision.get("reason") or "")[:50],
    }]
...
def _run_llm_agent_once(
    ...
    pick_slot_key: Optional[str] = None,
    ...
    sell_alert_types_by_symbol: dict[str, set[str]] = {}
    for a in alerts:
        symbol = (a.symbol or "").strip().upper()
        if not symbol:
            continue
        sell_alert_types_by_symbol.setdefault(symbol, set()).add((a.alert_type or "").strip())

    recent_agent_trades = [t for t in all_trades if _trade_source_bucket(getattr(t, "source", "")) == _agent_id_to_bucket(agent_id)]
    recent_lines = []
...
请给出你的决策："""
    ...
    try:
        ...
        with session_scope() as s:
            ...
            c.last_status = f"llm_error:{e}"
            c.updated_at = now
        return {"ok": True, "message": f"llm_error:{e}"}
    ...
    try:
        ...
        with session_scope() as s:
            c = _get_or_create_agent_config(s, agent_id)
            c.last_status = f"llm_parse_error:{text[:100]}"
            c.updated_at = now
        return {"ok": True, "message": f"llm_parse_error"}

    actions = _normalize_llm_actions(decision)
    ...
    for item in actions[:5]:
        ...
        if action == "sell" and symbol:
            ...
            with session_scope() as s:
                ...
                if trade is not None:
                    executed_trades.append(trade.model_dump())
                    last_action = f"SELL:{symbol}:{qty}@{sell_price:.2f}"
                    last_status = f"llm_executed_sell:{reason}"
        ...
        if action == "buy" and symbol:
            ...
            with session_scope() as s:
                ...
                if trading_now:
                    ...
                    committed_trade = trade
                    if trade is not None:
                        post_commit_events.append(("llm_buy_executed", symbol, f"qty={qty}@{buy_price:.2f} reason={reason}"))
                        executed_trades.append(trade.model_dump())
                        last_action = f"BUY:{symbol}:{qty}@{buy_price:.2f}"
                        last_status = f"llm_executed_buy:{reason}"
                        available_capital = max(0.0, available_capital - qty * buy_price)
                        held_symbols.add(symbol)
                        held_symbols_base.add(symbol_base)
                    else:
                    ...
                    if queued_order is not None:
                        ...
                        queued_orders.append(queued_order.model_dump())
                        last_action = f"QUEUE_BUY:{symbol}:{qty}@{buy_price:.2f}"
                        last_status = f"llm_queued_buy:{reason}"
                        available_capital = max(0.0, available_capital - qty * buy_price)
                        held_symbols.add(symbol)
                        held_symbols_base.add(symbol_base)
                    else:
    ...
    for trade in committed_trades:
        _send_feishu_trade_notify(trade.position_id, trade.side, trade.price, trade.quantity, trade.symbol, trade.name, trade.market, trade.source)
    if executed_trades:
        return {"ok": True, "message": "llm_trade_executed", "trades": executed_trades}
    if queued_orders:
        return {"ok": True, "message": "llm_trade_queued", "orders": queued_orders}
    return {"ok": True, "message": result_message}


def _agent_id_to_bucket(agent_id: str) -> str:
...
def _run_portfolio_agent_once(
    ...
    if agent_id == "b" or agent_type == "llm":
        with session_scope() as s:
            cfg = _get_or_create_agent_config(s, agent_id)
        return _run_llm_agent_once(agent_id, cfg, allow_new_pick=allow_new_pick, pick_slot_key=pick_slot_key, precomputed_alerts=precomputed_alerts)
    ...
    with session_scope() as s:
        ...
        if not candidates:
            ...
            if picked_trades:
                ...
                cfg.last_status = f"picked_new_stock:{len(picked_trades)}"
                cfg.updated_at = now
                result = {"ok": True, "message": "picked_new_stock", "trades": picked_trades}
            elif queued_orders:
                cfg.last_action = f"QUEUE_BUY:{queued_orders[-1]['symbol']}:{queued_orders[-1]['quantity']}@{queued_orders[-1]['trigger_price']:.2f}"
                cfg.last_status = f"queued_new_stock:{len(queued_orders)}"
                cfg.updated_at = now
                result = {"ok": True, "message": "queued_new_stock", "orders": queued_orders}
            elif executed_trade is not None:
                result = {"ok": True, "message": "trade_executed", "trade": executed_trade.model_dump()}
            else:
                post_commit_events.append(("candidate_trade_failed", None, None))
                cfg.last_status = "candidate_trade_skipped"
                cfg.updated_at = now
                result = {"ok": True, "message": "candidate_trade_skipped"}
    for event, symbol, detail in post_commit_events:
        _log_agent_pick_event(agent_id, slot_key, event, symbol=symbol, detail=detail)
    for trade in committed_trades:
        _send_feishu_trade_notify(trade.position_id, trade.side, trade.price, trade.quantity, trade.symbol, trade.name, trade.market, trade.source)
    return result or {"ok": True, "message": "candidate_trade_skipped"}


@app.get("/api/portfolio/agent/config", response_model=PortfolioAgentConfigResponse)
def get_portfolio_agent_config(agent_id: str = "a"):
...
@app.get("/api/portfolio/agent/pick-logs")
def get_portfolio_agent_pick_logs(agent_id: str = "a", limit: int = 50):
    agent_id = agent_id.strip() or "a"
    limit = max(1, min(200, int(limit or 50)))
    with session_scope() as s:
        rows = s.execute(
            select(PortfolioAgentPickLog)
            .where(PortfolioAgentPickLog.agent_id == agent_id)
            .order_by(PortfolioAgentPickLog.created_at.desc())
            .limit(limit)
        ).scalars().all()
        return [
            {
...
def get_portfolio_agent_status(agent_id: str = "a"):
    agent_id = agent_id.strip() or "a"
    summary = get_portfolio_summary()
    with session_scope() as s:
        cfg = _get_or_create_agent_config(s, agent_id)
    ...
    capital = float(cfg.capital or 10000000.0)
    managed_capital, managed_realized_pnl, managed_unrealized_pnl, managed_net_pnl = _compute_agent_managed_financials(trades, positions, agent_id, capital)
    auto_trade_count, auto_pick_count, auto_pick_success_count, auto_pick_closed_count, auto_pick_success_rate = _compute_agent_pick_kpis(trades, positions, agent_id)
    month_buy_amount = _compute_agent_month_buy_amount(trades, agent_id)
    capital_utilization_pct = (month_buy_amount / capital * 100.0) if capital > 0 else 0.0
...
def get_portfolio_returns():
    ...
    return _compute_period_returns(positions, summary, holdings, trades, pos_prices)
...
def update_portfolio_agent_config(req: PortfolioAgentConfigRequest):
    agent_id = getattr(req, "agent_id", "a") or "a"
    now = int(time.time())
    with session_scope() as s:
        cfg = _get_or_create_agent_config(s, agent_id)
...
def create_auto_trade(req: PortfolioAutoTradeRequest):
    ...
    with session_scope() as s:
        ...
        return PortfolioAutoTradeResponse(
            ...
            status="PENDING", source=at.source, created_at=now,
    ...
    with session_scope() as s:
        ...
        for at, p in rows:
            result.append(PortfolioAutoTradeResponse(
                ...
                status=at.status, source=(at.source or "auto_order"), created_at=at.created_at,
...
def _execute_live_auto_trade(at_id: str):
    import logging
    log = logging.getLogger("auto_trade")
    committed_trade: Optional[PortfolioTradeResponse] = None
    try:
        with session_scope() as s:
            ...
            if current_price <= 0 or trigger_price <= 0:
                return
            triggered = False
            if (at.side or "").strip().upper() == "BUY":
                triggered = current_price <= trigger_price
            elif (at.side or "").strip().upper() == "SELL":
                triggered = current_price >= trigger_price
            if not triggered:
                return
            qty = float(at.quantity or 0.0)
            committed_trade = _create_trade_at_price(p.id, at.side, qty, current_price, at.source or "auto_order", session=s)
            if committed_trade is None:
                return
            now = int(time.time())
            at.status = "EXECUTED"
            at.executed_at = now
...
def _process_live_auto_trades(market: str = "CN"):
    ...
    with session_scope() as s:
        pending = s.execute(
            ...
            .where(PortfolioPosition.market == market)
            .where(PortfolioAutoTrade.source.like("auto_strategy_%"))
        ).scalars().all()
        pending_ids = [at.id for at in pending]
    for at_id in pending_ids:
        _execute_live_auto_trade(at_id)
